...
**Repo hygiene (2026-07-21):** the root holds only active code + config (`mobile/`, `lifeos/`, `peak-fettle-agents/`, `marketing-site/`, `db/`, `migrations/`, `scripts/`, `legal/`, `sync-rules.yaml`, this file). ALL historical roadmaps, specs, prompts, tester feedback, and design mockups live in `docs/archive/` — put new dated planning docs there, never at root. Dead prototypes (Qt/QML app, `MyApp`, `peak-fettle-app`, `mind`, `ollama-agents`) were deleted; personal/untracked strays were moved outside the repo to `C:\Users\aavir\dev\pf-attic`.
...
**1. Local-first is the central invariant — free users make NO personal REST calls.** Tier branch point: `mobile/src/data/backup/tierPolicy.ts` (`isLocalFirst(user)` / `syncsToServer(user)`). FREE (`!is_paid`) = on-device SQLite only (`mobile/src/db/localDb.ts`); PRO = server + PowerSync. There is a tier-branched data layer — use it, never raw `api/*` from a screen/component: `mobile/src/data/{routines,profile,schedule}.ts`, and the `useWorkout*`/`useStreak`/`useInsights`/`useHealthMetrics` hooks. The #1 recurring bug was a screen/hook/component calling `getRoutine`/`getPercentile`/`getConstraints`/etc. directly on mount → for free users that's a failing/slow round-trip (infinite spinners, 500s, slow startup). **Review step:** `grep` new screens for raw `from '../api/` imports of personal-data endpoints; each needs an `isLocalFirst` branch. (Group weekly-signal is the ONE network call allowed on the free path.) **Since 2026-07-21 this is CI-enforced:** `peak-fettle-agents/server/scripts/invariant-sweep.js` (run in `.github/workflows/parse-sweep.yml`, or locally via `npm run invariants` in mobile/) fails any NEW screen/component that value-imports `api/*` outside its audited allowlist — audit the branching, then add the file to `LOCAL_FIRST_AUDITED` in that script. It also bans inlined unit-conversion literals (2.20462 / 2.54 / …) outside `units.ts` (see #2).

**2. Weight: fixed-point exact entry (v18) + exact kg, never the legacy kg×8.** Since schema **v18** (2026-07-21) the local `sets` table stores the EXACT typed value as integers: `weight_centi INTEGER` = typed value × 100 in the TYPED unit (50 lb → 5000, 82.5 kg → 8250) + `weight_unit TEXT` ('kg'|'lbs'). These are the DISPLAY/EDIT source of truth. `weight_kg REAL` (v3 exact kg) remains the CANONICAL computational value (1RM, volume, percentiles, sync); the old `weight_raw INTEGER` (kg×8, 0.125 resolution) is lossy and legacy-percentile-only. **Read** kg via `COALESCE(weight_kg, weight_raw/8.0)`; **write all three** on every set insert/update (see `useWorkout.ts` for the column list). Convert ONLY via `mobile/src/constants/units.ts`: `displayToCenti`+`displayToKg` to store, `formatSetWeight`/`setWeightToInputValue` to display/prefill (prefer these — they use the exact entry), `formatWeight` as kg fallback. **The old nearest-0.25-lb display rounding is DELETED** — it corrupted displays (186.7 → 186.75); never reintroduce it outside plate-increment stepping. Server: `weight_raw` (kg×8) is the only computational column (`weight_kg` is an API alias = raw/8); `weight_centi`/`weight_unit` mirror the exact entry once `migrations/20260721_sets_weight_centi.sql` is applied (routes/sets.js drift-degrades until then).
...
Active backlog: **TICKET-128…146** in `docs/archive/DEV_ROADMAP_2026-07-03-FEATURE-GAPS.md` — see its 🚦 STATUS block (as of 2026-07-04 only 140 [parked on founder go/no-go] and 146 [must run solo, last] remain). The TICKET-051…062 plan below is COMPLETE — historical reference only. Pick your lane by the model you are running:
...
Run order + per-ticket detail + the mandatory parse-sweep "definition of done" are in the **"⚙️ MODEL ROUTING FOR AGENT RUNS"** section of `docs/archive/DEV_ROADMAP_2026-05-25-LATE.md`. The HEAD-blob parse-sweep + `node --check` is required for every ticket on every model.
