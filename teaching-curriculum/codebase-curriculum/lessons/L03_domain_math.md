# L03: Domain Math — Strength Curves, Epley, and Percentile Ranking

**Domain:** Applied strength training biomechanics; statistical ranking; personalization models  
**Level:** Intermediate (Bloom L2–L5); assumes calculus comfort  
**Estimate:** 2.5–3.5 hours to complete all sections  
**Prerequisites:** L01 (Peak Fettle domain overview); L02 (C++ implementation); high school physics (logs, exponents)  
...
## 2. Pre-Lesson Survey (M1: Diagnostic Assessment)

Answer the following three questions *before* reading Section 3. Be honest about your confidence.
...
Peak Fettle uses:
```
strength_score = 300 + 3 × (e1rm_kg − 50)   [if e1rm ≥ 50]
strength_score = 0                          [if e1rm < 50]
```

This is intentionally *gentle*. At 100 kg bench (intermediate male, 75 kg BW), the score is:
```
300 + 3 × (100 − 50) = 300 + 150 = 450
```

At 120 kg (advanced), the score is:
```
300 + 3 × (120 − 50) = 300 + 210 = 510
```

The 20 kg jump (100 → 120) yields only a 60-point increase (450 → 510). This is intentional—the curve is *gentle* so beginners see frequent small wins (psychological boost), not rare large jumps.

**Real code (WorkoutTracker.cpp, implied implementation):**
```cpp
static double computeStrengthScore(double e1rmKg) {
    if (e1rmKg < 50.0) return 0.0;
    return 300.0 + 3.0 * (e1rmKg - 50.0);
}
```

**Elaboration (M6):**
The 0–1000 scale is arbitrary. You could use:
- Linear: 0–1000 linearly scaled to actual lift progression
- Exponential: larger gains for bigger lifts (e.g., 1000 = elite standard)
- Percentile-based: use the percentile rank as the score

Peak Fettle chose a linear-ish function (300 + 3×delta) because:
1. **Simplicity:** Easy to explain ("each additional 10 kg adds 30 points")
2. **Psychology:** Beginners see frequent small gains
3. **Calibration:** 100 kg bench ≈ 450 score is intuitive for intermediate

The floor (50 kg) avoids scoring partial/unsuccessful attempts.

**Retrieval check (M3):**
If someone benchmarks 50 kg, what's their strength score?
- (A) 0
- (B) 50
- (C) 300
- (D) 450

*Answer:* (C). `300 + 3 × (50 − 50) = 300 + 0 = 300`.

**Worked Example 4: Score Progression**
A user's bench press progress:
- Month 1: E1RM = 60 kg → Score = 300 + 3×10 = 330
- Month 3: E1RM = 75 kg → Score = 300 + 3×25 = 375 (+45 points)
- Month 6: E1RM = 90 kg → Score = 300 + 3×40 = 420 (+45 points)
- Month 9: E1RM = 105 kg → Score = 300 + 3×55 = 465 (+45 points)

Each 15 kg jump in 1RM yields the same 45-point increase in score—linear progress visibility. This is user-friendly.
...
4. **Estimate time for L04:**
   - L04 will cover the C++ implementation of the model (how the SQL math translates to code)
   - Recommend 2–3 hours
   - Schedule within 3–5 days (while the math is fresh)
...
6. **Gamified scoring:** 0–1000 score with gentle curve (300 + 3×(e1rm−50)) for psychological boost
