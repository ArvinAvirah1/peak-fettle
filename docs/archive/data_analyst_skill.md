...
| 2026-05-27 | strength_curve_model.md §4.5 (v2.1 layer) + compute_percentile.sql (sex-only fns) + lift_vectors_seed.sql (sex-only cols) — TICKET-052 | Re-uses v2 fits: Bielik 2024 (tier 1), Strength Level n>2M (tier 4), OpenPowerlifting (tier 3) population BW spread; NHANES adult anthropometry as outer bound for the ln-BW SD prior. No new external dataset — bodyweight is marginalised analytically out of the already-fitted v2 population model (Equation B). | (1) Marginal median = conditional median at the population geometric-mean BW: male squat 122 kg ≈ 1.5×BW allometric @85 kg ✓; bench 81.5 kg ≈ 180 lb median trained male ✓; deadlift 142.7 kg ✓. (2) Worked: male 100 kg bench → 71.9th sex-only pctile; female 60 kg bench → 72.4th ✓. (3) Sanity table P50/P90/P99/P99.9 per sex for the 5 base lifts smell-tested vs known absolute-load standards (squat/bench/deadlift/OHP) — P99.9 lands at elite raw-lift territory as intended for the Grand-Champ band. (4) Overall example squat140/bench100/dead180 (M) → 63.8/71.9/76.1 → mean 70.6. | Sex-only univariate model (Q3): each lift ranked vs same-sex trainees only, NO bodyweight/age/experience. mu_sex = pop_mu + α·ln(G/BW₀); sigma_sex = sqrt(α²·s² + pop_sigma²). Analyst BW priors (decision, not estimated): M G=85.0/s=0.17, F G=68.0/s=0.18 — re-fit on first-party data later. ADDITIVE: model_version stays 2; v2 columns/equations untouched; both sex rows carry the (m+f) pair; accessories inherit via ln(ratio) like pop_mu. Overall score = unweighted mean of per-lift sex-only pctiles; min 3 distinct lifts recommended before showing a tier (founder-tunable; overall_strength_percentile_detail() returns n_lifts + min_lifts_met). LIMITATIONS: tail (P99/P99.9, hence Diamond/Champ/GC cutoffs) sensitive to s; marginal-normal exact only if ln(BW) normal (real BW mildly right-skewed); sex-only ignores BW by design so a light elite-for-size lifter scores mid-pack. Feeds TICKET-053 (tier ladder). |
...
- v2.1 sex-only population BW priors (G, s per sex) are analyst defaults, not first-party estimates. Re-fit `G` and `s` from a representative same-sex bodyweight sample once available — they drive the upper-tail (P99/P99.9) and therefore the Diamond/Champ/Grand-Champ tier cutoffs (TICKET-053).
