...
const ts = require(
  '/sessions/youthful-jolly-franklin/mnt/Peak Fettle/mobile/node_modules/typescript'
);

const REPO = '/sessions/youthful-jolly-franklin/mnt/Peak Fettle/mobile';
...
(async () => {
  ...
  await test('runner idempotence: run twice = same version', async () => {
    ...
    eq(v1, 2, 'expected version 2:');
  });

  // 2. Fresh install reaches version 2
  await test('fresh install reaches user_version 2', async () => {
    const db = makeStubDb();
    eq(db._pragmas.user_version, 0, 'starts at 0:');
    await runMigrations(db);
    eq(db._pragmas.user_version, 2, 'should be 2 after migration:');
  });
