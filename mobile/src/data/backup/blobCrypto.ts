/**
 * blobCrypto — TICKET-094 Workstream B (Agent E)
 *
 * Pure AES-256-GCM encryption/decryption for the E2E backup blob.
 * Envelope format v1 per BACKUP_SUBSYSTEM_SPEC_2026-06-11.md §2.
 *
 * Dependencies: @noble/ciphers (gcm), @noble/hashes (scryptAsync).
 * NO React-Native or Expo imports at the module level — RNG is injected
 * for testability; the production default lazily requires expo-crypto.
 *
...
/** The exact on-disk envelope described in spec §2. */
export interface Envelope {
  ...
  ct: string;          // base64-encoded ciphertext of canonicalized ExportDoc
...
// Crockford's alphabet — 32 characters, no I/L/O/U ambiguity
const CROCKFORD_ALPHABET = '0123456789ABCDEFGHJKMNPQRSTVWXYZ';

/** Encode bytes to Crockford base32 (upper-case, no padding). */
...
/** Decode Crockford base32 string to bytes (normalization must be applied first). */
 ...
 * (Crockford base32 excludes I, L, O, U from its alphabet to prevent confusion.)
 */
...
function bytesToB64(bytes: Uint8Array): string {
  ...
  // Fallback for environments without Buffer (should not be reached in RN/Node)
  let binary = '';
 ...
 *
 * 120 bits / 5 bits-per-char = 24 characters → 6 groups of 4.
 * @param rng Injected RNG — defaults to expo-crypto.getRandomBytes.
 */
export function generateRecoveryCode(rng: RngFn = productionRng): string {
  // 15 bytes = 120 bits; Crockford encodes to exactly 24 chars (120/5 = 24)
  const bytes = rng(15);
  const encoded = crockfordEncode(bytes);
  // Pad to 24 chars if needed (should not be needed for 15-byte input)
 ...
 * 16-byte salt, then wrap the data key under it.  Returns the three b64 fields
 * that travel inside every envelope.  Call once when a new key+code is generated;
 * persist the result via keyStore.saveKeyWrap so it can be reused on every
 * subsequent backup without needing the recovery code again.
 *
 * @param dataKey      32-byte AES-256 data key
 * @param recoveryCode user's recovery code (normalized internally)
 * @param rng          Injected RNG — defaults to expo-crypto.getRandomBytes
 */
 ...
 * call.  Reusing the same salt/wrap_iv/wrapped_key across envelopes is sound:
 * the KEK+IV pair encrypts the same plaintext (dataKey) every time, producing
 * identical wrapped_key ciphertext — no new information leaks.
 *
...
export async function encryptBackup(
  ...
  // Fresh IV for the payload only — wrap fields come from the persisted KeyWrap.
 ...
 * Exposes this step separately so the restore path can re-save the key to the keychain.
 *
...
export async function unwrapDataKey(env: Envelope, code: string): Promise<Uint8Array> {
  try {
    ...
    // Decrypt: 48B input → 32B output (GCM strips the 16B tag)
