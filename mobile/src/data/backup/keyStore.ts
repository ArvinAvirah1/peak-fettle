/**
 * keyStore — TICKET-094 Workstream B (Agent E)
 *
 * Thin wrapper around expo-secure-store for the backup data key, plus an
 * AsyncStorage flag for recovery-code acknowledgement.
 *
 * Security rules:
...
const DATA_KEY_STORE_KEY = 'pf_backup_data_key_v1';

/** AsyncStorage key for the recovery-code acknowledgement flag. */
const RECOVERY_ACK_STORAGE_KEY = '@peak_fettle/recovery_code_ack';

/** SecureStore key for the JSON-serialized KeyWrap (ciphertext — safe at rest). */
const KEY_WRAP_STORE_KEY = 'pf_backup_key_wrap_v1';
...
/**
 * Import the KeyWrap type from blobCrypto.  We use an inline structural type
 * here to avoid a circular-import concern — the shape is identical to KeyWrap.
 */
