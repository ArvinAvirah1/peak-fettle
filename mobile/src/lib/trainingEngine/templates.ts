// templates.ts — Training Engine v1 (mobile port)
// 7 disciplines × 3 tiers — ported faithfully from server/lib/trainingEngine/templates.js
// Pure data; no DB access.
...
export interface Template {
  idealDays: number;
  deloadEvery: number;
  progression: { model: 'linear' | 'dup' | 'block'; weeklyRule: string };
  sessions: SessionTemplate[];
}

export type DisciplineTemplates = {
  beginner: Template;
...
const templates: Record<string, DisciplineTemplates> = {
  general_strength:              generalStrength as unknown as DisciplineTemplates,
  general_strength_hypertrophy:  generalStrengthHypertrophy as unknown as DisciplineTemplates,
  powerlifting:                  powerlifting as unknown as DisciplineTemplates,
  weightlifting:                 weightlifting as unknown as DisciplineTemplates,
  running:                       running as unknown as DisciplineTemplates,
  cycling:                       cycling as unknown as DisciplineTemplates,
  swimming:                      swimming as unknown as DisciplineTemplates,
  other_mixed:                   otherMixed as unknown as DisciplineTemplates,
};
...
export function getTemplate(
  ...
  const useHypertrophy =
    normDisc === 'general_strength' && goal === 'hypertrophy';
  const disciplineTemplates = useHypertrophy
    ? (generalStrengthHypertrophy as unknown as DisciplineTemplates)
    : (templates[normDisc] || (generalStrength as unknown as DisciplineTemplates));

  const normTier = (tier || 'beginner').toLowerCase() as keyof DisciplineTemplates;
  return (disciplineTemplates[normTier] as Template) ||
         (disciplineTemplates['beginner'] as Template) ||
         null;
}

export default { getTemplate, ...templates };
